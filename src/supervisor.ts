  ...
  const bootstrap =
    ...
    (cfg.threadAgents
      ? "\n[thread routing] Mattermost references are mm:channel:post. Ordinary roots are yours to triage. " +
        ...
        "On startup run bin/thread-control approval-list for compact unresolved handoff summaries. Thereafter use approval notifications; do not poll an unchanged list. " +
        "For a handoff requiring a decision, run bin/thread-control approval-read <id> to read its full original receipts and exact scope. " +
        ...
        "Verify actual content approval for that exact repo, PR URL and starting head. Approval notifications arrive when workerBusy=false, including surviving CLI locks after restart. If still busy, park and wait for that notification. Then use bin/thread-control approval-grant <handoff-id> <original-human-approval-reference> '<receipts JSON array from approval-read>'. If the receipt snapshot changed, read the new messages before retrying. This resumes the bound agent with authority to run final review and merge that PR after CLEAN. " +
