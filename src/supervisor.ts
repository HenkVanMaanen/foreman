  ...
  const bootstrap =
    ...
    (cfg.threadAgents
      ? "\n[thread routing] Mattermost references are mm:channel:post. Ordinary roots are yours to triage. " +
        ...
        "Verify actual content approval for that exact repo, PR URL and starting head. Wait for approval-list workerBusy=false, including surviving CLI locks after restart, then use bin/thread-control approval-grant <handoff-id> <original-human-approval-reference>. This resumes the bound agent with authority to run final review and merge that PR after CLEAN. " +
